...
describe 'redis' do
  ...
  describe 'Test standard installation' do
    ...
    it { should contain_file('redis.conf').with_ensure('present') }
  end
  ...
  describe 'Test standard installation with monitoring and firewalling' do
    ...
    it 'should monitor the process' do
      content = catalogue.resource('monitor::process', 'redis_process').send(:parameters)[:enable]
      content.should == true
    end
    it 'should place a firewall rule' do
      content = catalogue.resource('firewall', 'redis_tcp_42').send(:parameters)[:enable]
      content.should == true
    end
  end
  ...
  describe 'Test decommissioning - disableboot' do
    ...
    it { should contain_package('redis').with_ensure('present') }
    it { should_not contain_service('redis').with_ensure('present') }
    it { should_not contain_service('redis').with_ensure('absent') }
    it 'should not enable at boot Service[redis]' do should contain_service('redis').with_enable('false') end
    it { should contain_file('redis.conf').with_ensure('present') }
  ...
  describe 'Test customizations - template' do
    let(:params) { {:template => "redis/spec.erb" , :options => { 'opt_a' => 'value_a' } } }

    it 'should generate a valid template' do
      content = catalogue.resource('file', 'redis.conf').send(:parameters)[:content]
  ...
  describe 'Test customizations - source' do
    let(:params) { {:source => "puppet://modules/redis/spec" , :source_dir => "puppet://modules/redis/dir/spec" , :source_dir_purge => true } }

    it 'should request a valid source ' do
      content = catalogue.resource('file', 'redis.conf').send(:parameters)[:source]
      content.should == "puppet://modules/redis/spec"
    end
    it 'should request a valid source dir' do
      content = catalogue.resource('file', 'redis.dir').send(:parameters)[:source]
      content.should == "puppet://modules/redis/dir/spec"
    end
    it 'should purge source dir if source_dir_purge is true' do
      content = catalogue.resource('file', 'redis.dir').send(:parameters)[:purge]
      content.should == true
    end
  end
  ...
  describe 'Test Puppi Integration' do
    ...
    it 'should generate a puppi::ze define' do
      content = catalogue.resource('puppi::ze', 'redis').send(:parameters)[:helper]
  ...
  describe 'Test Monitoring Tools Integration' do
    ...
    it 'should generate monitor defines' do
      content = catalogue.resource('monitor::process', 'redis_process').send(:parameters)[:tool]
  ...
  describe 'Test Firewall Tools Integration' do
    ...
    it 'should generate correct firewall define' do
      content = catalogue.resource('firewall', 'redis_tcp_42').send(:parameters)[:tool]
  ...
  describe 'Test OldGen Module Set Integration' do
    ...
    it 'should generate puppi resources ' do 
      content = catalogue.resource('puppi::ze', 'redis').send(:parameters)[:ensure]
  ...
  describe 'Test params lookup' do
    let(:facts) { { :redis_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
  ...
  describe 'Test params lookup' do
    let(:facts) { { :monitor => false , :redis_monitor => true , :ipaddress => '10.42.42.42' } }
    let(:params) { { :port => '42' } }
